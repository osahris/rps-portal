...
docker-compose run --rm -u root --no-deps nextcloud chown -R www-data:www-data /var/www/html
...
docker-compose run --rm -u root --no-deps nextcloud chown -R www-data:www-data /srv/nextcloud/data
